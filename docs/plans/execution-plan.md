...
| M3 | Viewer core lifecycle/layout/default state/error fallback | M2 | `src/components/NglViewport.vue`, `src/components/ViewerTopBar.vue`, `src/components/ControlsPanel.vue`, `src/viewer/nglStage.ts`, viewer shell state store/module | 1. `/viewer` shows explicit loading state then ready state. 2. Default state: `3fly_cryst_lig`, baseline on, refined off, no maps visible. 3. Camera baseline snapshot constant exists and `Reset view` target is defined. 4. Resize preserves camera state. 5. Unmount/remount does not duplicate listeners. 6. Failure-path: forced stage init failure shows fallback message + retry/navigation option. | Pass only if viewer core acceptance checks in `docs/specs/viewer-core-spec.md` are satisfied and failure fallback works. | Risk: lifecycle leaks. Mitigation: teardown assertions and remount smoke test in milestone gate. |
