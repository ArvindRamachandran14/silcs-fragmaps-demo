...
- M5 sliced-preview exception (locked for this project):
  - M5 is delivered in six slices (`M5.1`..`M5.6`) and uses per-slice design gates.
...
flowchart TD
    ...
    M52 --> M53["M5.3 Advanced + Exclusion Behavior"]
...
| M5 | FragMap controls delivered as sliced milestones (`M5.1`..`M5.6`): panel shell, primary toggle engine, advanced/exclusion behavior, per-map iso controls, bulk actions, reliability hardening | M4B | `src/components/FragMapControls.vue`, map store/actions, map rep update logic in `src/viewer/reps.ts` | 1. Per-slice Prompt A + Prompt B flow with explicit approval for each slice. 2. M5.1 shell includes right-panel tab framework (`FragMap` + `Ligand`) with `FragMap` active by default and `Ligand` tab preserving existing M4B ligand controls. 3. Panel shows Primary 3 + expandable Advanced list with canonical labels/colors. 4. All maps hidden by default; lazy load on first toggle; cache reused on repeat toggles. 5. Per-map iso controls enforce numeric contract (`step`, `min`, `max`, precision, clamp/revert). 6. Exclusion map is visibility-toggleable with disabled iso and fixed gray triangulated wireframe style. 7. Bulk actions (`Hide all`, `Reset defaults`, `Reset view`) match spec. 8. Failure-path: row-level map failure isolation with retry; unaffected rows remain stable. | Pass only if FragMap acceptance checks in `docs/specs/fragmap-controls-spec.md` pass, including no global iso control and row-level failure isolation. | Risk: async race instability during rapid toggles/iso edits. Mitigation: request-id or single-flight guards per map row. |
...
| M5.3 | Advanced rows + Exclusion Map fixed behavior (visibility-toggleable, fixed gray triangulated wireframe style, no editable iso). | M5.2 | Advanced rows and exclusion behavior match spec without regressing M5.2. |
