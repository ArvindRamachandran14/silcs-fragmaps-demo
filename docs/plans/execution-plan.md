...
- M5 sliced-preview exception (locked for this project):
  - Required M5 delivery is eight slices (`M5.1`, `M5.2`, `M5.2a`, `M5.2b`, `M5.3`, `M5.4`, `M5.5`, `M5.6`) and uses per-slice design gates.
...
flowchart TD
    ...
    M55 --> M56["M5.6 Reliability Hardening + Final Gate"]
...
| M5.6 | Reliability hardening (row-level failure isolation, retry, async race guards) + final M5 gate evidence. | M5.5 | Failure-path checks pass and `validate:m5` gate is green. |
