...
- M5 sliced-preview exception (locked for this project):
  - Required M5 delivery is nine slices (`M5.1`, `M5.2`, `M5.2a`, `M5.2b`, `M5.3`, `M5.4`, `M5.5`, `M5.5a`, `M5.6`) and uses per-slice design gates.
  - `M5.2c` is exploratory/deferred parity investigation only (non-blocking, not a required milestone gate).
  - Preview packet location: `docs/screenshots/Design_previews/m5-fragmap-controls/`.
  - Packet structure: one front page (`README.md`/index) plus one preview page per slice.
  - Each slice Prompt A may use a single multi-panel SVG page for that slice as long as default/loading/empty/error/success states are covered on that page.
  - Prompt B for a slice cannot start until that slice has explicit `APPROVED UI PREVIEW`.
...
flowchart TD
    ...
    M4B --> M51["M5.1 FragMap Panel Shell"]
    M51 --> M52["M5.2 Primary-3 Toggle Engine"]
    M52 --> M52A["M5.2a Wireframe Rendering Pass"]
    M52A --> M52B["M5.2b Protein Visibility Toggle"]
    M52B --> M53["M5.3 Advanced + Exclusion Behavior"]
    M53 --> M54["M5.4 Per-Map Iso Controls"]
    M54 --> M55["M5.5 Bulk Actions"]
    M55 --> M55A["M5.5a Reset Defaults (ISO-only)"]
    M55A --> M56["M5.6 Reliability Hardening + Final Gate"]
    M3 --> M6["M6 Overview Page"]
    M56 --> M7["M7 Performance Instrumentation + AC Validation Evidence"]
    M6 --> M7
...
| M4A | Ligand core workflow (`3fly_cryst_lig` only): 4 pose states, zoom, empty/error handling | M3 | `src/components/LigandControls.vue`, ligand store/actions, pose component management in `src/viewer/reps.ts` | 1. Design Preview Gate passed (artifacts + explicit `APPROVED UI PREVIEW`). 2. UI contains baseline/refined checkboxes, legend (both-visible), and zoom action. 3. Supports baseline-only/refined-only/both-visible/both-unchecked. 4. Both-unchecked is represented directly by unchecked pose toggles (helper guidance optional). 5. Switching pose states is in-place (no reload). 6. Failure-path: per-pose load failure disables only affected pose and shows toast. | Pass only if M4A checks in `docs/specs/ligand-workflow-spec.md` pass for single-ligand scope (`3fly_cryst_lig`). | Risk: pose-toggle race conditions. Mitigation: cancel stale load intents and preserve previous valid rendered state on hard failures. |
...
| M5 | FragMap controls delivered as required sliced milestones (`M5.1`, `M5.2`, `M5.2a`, `M5.2b`, `M5.3`, `M5.4`, `M5.5`, `M5.5a`, `M5.6`): panel shell, primary toggle engine, wireframe rendering pass, protein visibility toggle, advanced/exclusion behavior, per-map iso controls, bulk actions, reset-default semantics refinement, reliability hardening. Optional exploratory `M5.2c` parity work is non-blocking and deferred unless explicitly promoted. | M4B | `src/components/FragMapControls.vue`, map store/actions, map rep update logic in `src/viewer/reps.ts` | 1. Per-slice Prompt A + Prompt B flow with explicit approval for each required slice. 2. M5.1 shell includes right-panel tab framework (`FragMap` + `Ligand`) with `FragMap` active by default and `Ligand` tab preserving existing M4B ligand controls. 3. Panel shows Primary 3 + expandable Advanced list with canonical labels/colors. 4. All maps hidden by default; lazy load on first toggle; cache reused on repeat toggles. 5. M5.2a applies triangulated wireframe rendering for all FragMaps, including fixed gray `Exclusion Map` wireframe style. 6. M5.2b adds a protein-cartoon visibility toggle (`on` by default) without changing map/ligand state contracts. 7. Per-map iso controls enforce numeric contract (`step`, `min`, `max`, precision, clamp/revert). 8. Exclusion map is visibility-toggleable with disabled iso. 9. Bulk actions expose `Hide all` and `Reset defaults`; panel does not include camera-reset action. 10. M5.5a refines `Reset defaults` to reset per-map iso values only and keep map visibility unchanged. 11. Failure-path: row-level map failure isolation with retry; unaffected rows remain stable. | Pass only if FragMap acceptance checks in `docs/specs/fragmap-controls-spec.md` pass, including no global iso control and row-level failure isolation. | Risk: async race instability during rapid toggles/iso edits. Mitigation: request-id or single-flight guards per map row. |
...
## 3.1 M5 Slice Plan (Locked)
| Slice ID | Scope boundary | Depends on | Exit condition |
|---|---|---|---|
| M5.1 | FragMap panel shell only (Primary/Advanced sections, canonical labels/colors, default all-hidden state) plus right-panel two-tab framework (`FragMap` + `Ligand`) with `FragMap` active by default and `Ligand` tab preserving existing M4B controls. No runtime map load/toggle wiring. | M4B | Prompt A/B complete for M5.1 with no M1-M4B regressions. |
| M5.2 | Primary-3 visibility engine only (toggle, lazy first load, cache reuse, camera preserved). | M5.1 | Primary rows switch reliably in-place and reuse cached handles. |
| M5.2a | Wireframe rendering pass only (switch map representation styling to triangulated wireframe, including fixed gray `Exclusion Map` wireframe). No new control behavior. | M5.2 | Wireframe rendering contract is implemented without regressing M5.2 behavior. |
| M5.2b | Protein visibility toggle only (`Protein cartoon` show/hide in FragMap tab; default on). No map toggle/iso behavior changes. | M5.2a | Protein visibility can be toggled in-place with no route reload and no regression to M5.2/M5.2a contracts. |
| M5.3 | Advanced rows + Exclusion Map fixed behavior (visibility-toggleable, fixed gray triangulated wireframe style, no editable iso). | M5.2b | Advanced rows and exclusion behavior match spec without regressing M5.1-M5.2b. |
| M5.4 | Per-map iso controls only for iso-adjustable rows (`step`, `min`, `max`, precision, clamp/revert). | M5.3 | Numeric iso contract matches spec in-place with no M5.1-M5.3 regressions. |
| M5.5 | Bulk actions only (`Hide all`, `Reset defaults`). | M5.4 | Bulk actions match spec in-place with no M5.1-M5.4 regressions. |
| M5.5a | `Reset defaults` semantics refinement only: reset per-map iso values to defaults while leaving visibility unchanged. | M5.5 | Reset action is iso-only, preserves current visibility, and introduces no M5.1-M5.5 regressions. |
| M5.6 | Reliability hardening (row-level failure isolation, retry, async race guards) + final M5 gate evidence. | M5.5a | Failure-path checks pass and `validate:m5` gate is green. |
